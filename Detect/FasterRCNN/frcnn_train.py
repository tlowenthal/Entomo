...
class Cust(torch.utils.data.Dataset):
    def __init__(self, image_path, boxes, indexes):
...
train_dl = torch.utils.data.DataLoader(Cust(image_path,boxes,train),batch_size=16,shuffle=True,collate_fn=cust_coll,pin_memory=True if torch.cuda.is_available() else False)

val_dl = torch.utils.data.DataLoader(Cust(image_path,boxes,val),batch_size=8,shuffle=True,collate_fn=cust_coll,pin_memory=True if torch.cuda.is_available() else False)
